...
Object的每个实例都具有下列属性和方法。

- constructor: 保存着用于创建当前对象的函数， 即指向他的构造函数。
...
对于对象转为数字或者字符串，都会默认调用valueOf()， 如果结果为NaN，则再调用toString()方法，然后再调用其他规则。
...
- `arguments`是你传入参数的集合， 不是你定义参数的集合
