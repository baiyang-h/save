...
```
不过在严格模式下`arguments。callee`会报错， 所以我们可以使用函数表达式的形式来递归，这种方法在严格模式和非严格模式下都可以；
```js
...
无论什么时候在函数中访问一个变量时，就会从作用域链中搜索具有相应名字的变量，一般来讲，当函数执行完毕后，局部活动变量就会销毁，内存中仅保存全局作用域，但是我们可以使用闭包来延迟局部变量的声明周期。
```js
function fnA() {
    let a = 111;
    return function() {
